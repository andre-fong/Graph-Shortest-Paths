...
bool decreasePriority(MinHeap* heap, int id, int newPriority) {
	HeapNode node = nodeAt(heap, indexOf(heap, id));
	
	if (node.priority <= newPriority) return false;
...
MinHeap* newHeap(int capacity) {
	...
	new->arr = (HeapNode *)calloc((capacity + 1), sizeof(HeapNode));	// allocate for capacity and empty index 0
	new->indexMap = (int *)calloc(capacity, sizeof(int));
